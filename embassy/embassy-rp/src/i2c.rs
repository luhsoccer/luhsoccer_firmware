...
impl<'d, T: Instance> I2c<'d, T, AsyncSlave> {
    pub fn new_async_slave(
        peri: impl Peripheral<P = T> + 'd,
        scl: impl Peripheral<P = impl SclPin<T>> + 'd,
        sda: impl Peripheral<P = impl SdaPin<T>> + 'd,
        ...
        unsafe {
            let i2c = T::regs();
            i2c.ic_enable().modify(|w| w.set_enable(false));
            i2c.ic_sar().modify(|w| w.set_ic_sar(u16::from(addr)));
            i2c.ic_con().modify(|w| {
                w.set_ic_slave_disable(false);
                w.set_master_mode(false);
            });
            i2c.ic_enable().modify(|w| w.set_enable(true));
        }
        T::Interrupt::unpend();
    ...
    pub async fn update(&mut self, mut f: impl FnMut(Action, Option<u8>) -> Option<u8>) {
        let p = T::regs();
        let irqs = self
            .wait_on(
                |_me| unsafe {
                    let irqs = p.ic_raw_intr_stat().read();
                    if irqs.tx_abrt() || irqs.start_det() || irqs.stop_det() || irqs.rx_full() || irqs.rd_req() {
                        Poll::Ready(irqs)
                    } else {
                        Poll::Pending
                    }
                ...
                |_me| unsafe {
                    p.ic_tx_tl().modify(|w| w.set_tx_tl(1));
            ...
            .await;
        if irqs.tx_abrt() {
            let _ = self.read_and_clear_abort_reason();
        }
        if irqs.start_det() {
            unsafe {
        ...
        if irqs.rx_full() {
            let byte = unsafe { p.ic_data_cmd().read().dat() };
            f(Action::Receive, Some(byte));
        }
        if irqs.rd_req() {
            unsafe {
                p.ic_clr_rd_req().read();
            }
            let byte = f(Action::Request, None).unwrap_or_default();
            unsafe {
...
pub struct Blocking;
pub struct Async;
pub struct AsyncSlave;

impl_mode!(Blocking);
impl_mode!(Async);
impl_mode!(AsyncSlave);
